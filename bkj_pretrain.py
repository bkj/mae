...
for epoch in range(1600, 2000):
    ...
    for batch_idx, (x, _) in enumerate(gen):
        ...
        log_writer.add_scalar('train_loss', float(loss), epoch * len(dl_train) + batch_idx)
        log_writer.add_scalar('train_lr',   float(lr),   epoch * len(dl_train) + batch_idx)
    ...
    if epoch % 10 == 0:
        with torch.inference_mode():
            ...
            clf       = LinearSVC().fit(X_valid, y_valid)
            valid_acc = (clf.predict(X_valid) == y_valid).mean()
            test_acc  = (clf.predict(X_test) == y_test).mean()
            
            log_writer.add_scalar('valid_acc', float(valid_acc), epoch * len(dl_train))
            log_writer.add_scalar('test_acc',  float(test_acc),  epoch * len(dl_train))
            log_writer.flush()
...
torch.save(state_dict, 'cifar10_model0.pt')
